...
export const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-4 shadow-lg' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 group">
            <img src="/assets/LOGO PNG.png" alt="Syntigra Logo" className="h-10 w-auto group-hover:scale-105 transition-transform" />
      ...
      {/* Mobile Nav */}
      {isOpen && (
        <div className="md:hidden glass-panel border-t border-slate-200 dark:border-slate-700">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-3 py-2 rounded-md text-base font-medium text-slate-600 dark:text-slate-300 hover:text-syntigra-blue hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                {link.name}
              </Link>
            ))}
            <Link
              to="/contact"
              onClick={() => setIsOpen(false)}
              className="block px-3 py-2 mt-4 text-center rounded-md bg-syntigra-blue text-white font-medium"
            >
              Book Consultation
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div className="col-span-1 md:col-span-1">
            <div className="flex items-center space-x-2 mb-6">
              <img src="/assets/LOGO PNG.png" alt="Syntigra Logo" className="h-8 w-auto" />
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li><Link to="/industries" className="text-slate-400 hover:text-syntigra-cyan text-sm">Industries</Link></li>
              <li><Link to="/cases" className="text-slate-400 hover:text-syntigra-cyan text-sm">Case Studies</Link></li>
              <li><Link to="/security" className="text-slate-400 hover:text-syntigra-cyan text-sm">Security</Link></li>
              <li><Link to="/contact" className="text-slate-400 hover:text-syntigra-cyan text-sm">Contact</Link></li>
            </ul>
          </div>
